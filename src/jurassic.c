...
void jur_kernel(ctl_t const *const ctl, atm_t *atm, obs_t *obs, gsl_matrix *k) {
	int iqa[N];
	jur_formod(ctl, atm, obs, NULL, 1);													 // Compute radiance for undisturbed atmospheric data
	// Compose vectors
	...
	//	#pragma omp parallel default(shared)
	{
		...
		for(int j = 0; j < (int) n; j++) {							// Loop over state vector elements
			...
			jur_formod(ctl, atm1, obs1, NULL, 1);											// Compute radiance for disturbed atmospheric data
