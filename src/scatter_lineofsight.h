...
void jur_add_aerosol_layers(ctl_t *ctl,
  ...
  los_aero = (los_t*) malloc(sizeof(los_t));
  ...
  jur_geo2cart(los_aero->z[0], los_aero->lon[0],los_aero->lat[0], x1);
  jur_geo2cart(los_aero->z[1], los_aero->lon[1],los_aero->lat[1], x2);
  los_aero->ds[0]= 0.5 * (DIST(x1,x2));
  for(ip=1; ip<los_aero->np-1; ip++){
    jur_geo2cart(los_aero->z[ip-1], los_aero->lon[ip-1],los_aero->lat[ip-1], x1);
    jur_geo2cart(los_aero->z[ip], los_aero->lon[ip],los_aero->lat[ip], x2);
    jur_geo2cart(los_aero->z[ip+1], los_aero->lon[ip+1],los_aero->lat[ip+1], x3);
...
void jur_sca_tangent_point(los_t *los,
  ...
  /* Limb... */
    ...
    x=-b/(2*a);
    *tpz=a*x*x+b*x+c;
    jur_geo2cart(los->z[ip-1], los->lon[ip-1], los->lat[ip-1], v0);
    jur_geo2cart(los->z[ip+1], los->lon[ip+1], los->lat[ip+1], v2);
    for(i=0; i<3; i++)
