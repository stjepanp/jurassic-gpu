...
void scatter_read_tbl(ctl_t *ctl,
	      tbl_t *tbl) {
...
void init_tbl(ctl_t const *ctl, trans_table_t *tbl) {
    ...
    printf("DEBUG kaj se dogadja\n");
    if(1) {
        printf("DEBUG ole ole citam tablicu na scattering nacin\n");
        tbl_t* t = (tbl_t*)malloc(sizeof(tbl_t));
        scatter_read_tbl(ctl, t);
        for(int ng=0; ng<NG; ng++)
          for(int tblnp=0; tblnp<TBLNP; tblnp++)
            for(int tblnt=0; tblnt<TBLNT; tblnt++)
              for(int tblnu=0; tblnu<TBLNU; tblnu++)
                for(int nd=0; nd<ND; nd++) {
                  tbl->np[ng][nd] = t->np[ng][nd];
                  tbl->nt[ng][tblnp][nd] = t->nt[ng][nd][tblnp];
                  tbl->nu[ng][tblnp][tblnt][nd] = t->nu[ng][nd][tblnp][tblnt];
                  tbl->p[ng][tblnp][nd] = t->p[ng][nd][tblnp];
                  tbl->t[ng][tblnp][tblnt][nd] = t->t[ng][nd][tblnp][tblnt];
                  tbl->u[ng][tblnp][tblnt][tblnu][nd] = t->u[ng][nd][tblnp][tblnt][tblnu];
                  tbl->eps[ng][tblnp][tblnt][tblnu][nd] = t->eps[ng][nd][tblnp][tblnt][tblnu];
                }
        for(int tblns=0; tblns<TBLNS; tblns++)
          for(int nd=0; nd<ND; nd++) { 
            tbl->st[tblns] = t->st[tblns];
            tbl->sr[tblns][nd] = t->sr[nd][tblns];
          }
        return;
    }
    if (ctl->read_binary) {
